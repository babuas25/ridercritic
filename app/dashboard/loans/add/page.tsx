...
import { createLoanOffer } from '@/lib/loans'
import { LoanOffer } from '@/types/loan'

export default function AddLoanOfferPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!session?.user?.id) {
      alert('You must be logged in to create a loan offer')
      return
    }

    try {
      setLoading(true)
      await createLoanOffer(formData as LoanOffer, session.user.id)
    ...
    <DashboardLayout>
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-3xl font-bold text-gray-900 dark:text-white">Add Loan Offer</h1>
            <p className="text-gray-600 dark:text-gray-400">
              Create a new motorcycle loan offer
            </p>
          </div>
          <Button 
            variant="outline"
        ...
        <form onSubmit={handleSubmit}>
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Fill in the details for the new loan offer
              </CardDescription>
            </CardHeader>
